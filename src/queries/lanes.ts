...
export const getLanesWithTicketsAndTags = async (pipelineId: string) => {
    const response = await db.lane.findMany({
        ...
        include: {
            tickets: {
                orderBy: { order: "asc" },
                include: { tags: true, customer: true, assigned: true },
            },
        },
    });
...
export const updateLanesOrder = async (lanes: Lane[]) => {
    try {
        const updateLanes = lanes.map((lane) =>
            db.lane.update({ where: { id: lane.id }, data: { order: lane.order } })
        );
...
export const upsertLane = async (lane: Prisma.LaneUncheckedCreateInput) => {
    let order: number;
    if (!lane.order) {
        const lanes = await db.lane.findMany({ where: { pipelineId: lane.pipelineId } });
        order = lanes.length;
    } else {
        order = lane.order;
    }
    const response = await db.lane.upsert({
        ...
        update: { name: lane.name, color: lane.color, order: lane.order },
        create: { name: lane.name, color: lane.color, pipelineId: lane.pipelineId, order },
    });
    return response;
};
