...
describe("Funnel URL builder — advanced", () => {
  it("default scheme is http when env undefined", () => {
    expect(normalizeScheme(undefined)).toBe("http");
  });

  it("empty string scheme defaults to http", () => {
    expect(normalizeScheme("")).toBe("http");
  });

  it("scheme with trailing colon is stripped", () => {
    expect(normalizeScheme("https:")).toBe("https");
  });

  it("full URL with all params", () => {
  ...
  it("leading slash in pathName is stripped", () => {
    const url = buildPublishedFunnelPageUrl({
      subDomainName: "sale",
      pathName: "/pricing",
      scheme: "http",
  ...
  it("default domain falls back to localhost:3000", () => {
    const url = buildPublishedFunnelPageUrl({
      subDomainName: "demo",
      pathName: "home",
    });
  ...
  it("whitespace in subDomainName is trimmed", () => {
    const url = buildPublishedFunnelPageUrl({
